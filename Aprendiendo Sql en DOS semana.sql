create database Ap_sql_2_semanas
go

use Ap_sql_2_semanas
go
...
--12.  Clave primaria Primer problema: Trabaje con la tabla "libros" de una librería.
--1. Elimine la tabla si existe
...
/*15 - Truncate table, primer problema: Un instituto de enseñanza almacena los datos de sus estudiantes en una tabla llamada "alumnos".
...
/* 17 - Tipo de dato (texto), primer problema:
...
create rule RG_vehiculos_tipo_2 as @tipo in ('a','c','m');
...
alter table vehiculos add constraint CK_vehiculos_llegada_salida check(horallegada<=horasalida);
...
add constraint DF_vehiculos_tipo default 'b' for tipo;
...
"sp_helpconstraint". Muestra 1 restricción "check", 1 restricción "default" y 4 reglas asociadas.*/
exec sp_helpconstraint vehiculos;
...
que solamente puedan ingresarse los caracteres "a", "c" y "m":*/
create rule RG_vehiculos_tipo as @tipo in ('a','c','m');
...
/*Note que es más conveniente emplear, en este caso, el operador ""between". Ver solución*/
...
--24- Verifique que ya no existe en la base de datos. Ver solución
exec sp_help VP_cero;

-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

/*Un comercio guarda la información de sus ventas en una tabla llamada "ventas" en la que se registra 
diariamente, el número de venta, el monto total de la compra, el tipo de pago (c=contado, t=tarjeta) y el nombre del vendedor.
...
grouping(estadocivil) as 'resumen estado civil' from empleados group by sexo,estadocivil with rollup;

/*6- Realice la misma consulta anterior pero con "cube" en lugar de "rollup", distinguiendo los 
valores de resumen y de detalle. */

select sexo,estadocivil, count(*) as cantidad, grouping(sexo) as 'resumen sexo',
grouping(estadocivil) as 'resumen estado civil' from empleados group by sexo,estadocivil with cube;
...
/*6- Obtenga el total de los clientes que viven en calle "San Martin" (where), agrupados por provincia (group by), teniendo en cuenta todos los valores (all),
de aquellas ciudades que tengan menos de 2 clientes (having) y omitiendo la fila correspondiente a la ciudad de "Cordoba" (having) (4 filas devueltas) */
select ciudad, count(*)
from clientes
...
Una agencia matrimonial almacena la información de sus clientes de sexo femenino en una tabla llamada "mujeres" y 
en otra la de sus clientes de sexo masculino llamada "varones"
...
--2- Cree las tablas con las restricciones necesarias*/
...
profesor que lo instruirá. 5 registros.*/
...
Primer problema: Un comercio almacena los datos de sus empleados en una tabla denominada "empleados".
...
insert into empleados default values; No se puede porque el campo "tipo_legajo" no admite valores nulos y no tiene definido un valor por defecto.*/
...
Primer problema: Un comercio almacena los datos de sus empleados en una tabla denominada "empleados" y en otra 
...
--10- Cree la tabla "clientes" con 3 campos: nombre (30 caracteres), añoingreso (tipo_año) y domicilio (30 caracteres):
create table clientes(
	documento char(8),
	nombre varchar(30),
	añoingreso tipo_año
...
-- Defina un nuevo tipo de dato llamado "tipo_año". Primero debe eliminarlo, si existe para volver a crearlo:
...
Primer problema: Trabaje con una tabla llamada "empleados".
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/* Subconsultas como expresión, Primer problema: Un profesor almacena el documento, nombre y la nota final de cada alumno de su clase en una tabla 
llamada "alumnos".
...
select documento ,nombre, nota from alumno where nota=(select nombre,max(nota) from alumnos);
...
/* Primer problema: Una empresa tiene registrados sus clientes en una tabla llamada "clientes", también tiene una tabla 
...
y la cantidad de cuotas pagas (desde 0 hasta 10 que es el total por todo el año), y una tabla denominada "socios" en la que guarda los datos personales de cada socio.*/
...
lo borra de la inscripción de todos los deportes. Elimine todos los socios que no pagaron ninguna cuota en algún deporte.
...
en la cual almacena el nombre del deporte, el nombre del profesor que lo dicta, el día de la semana  que se dicta y el costo de la cuota mensual.
...
@sueldo decimal(6,2) as select nombre,apellido,sueldo from empleados where sueldo>=@sueldo;
...
create procedure pa_empleados_actualizar_sueldo @sueldoanterior decimal(6,2), @sueldonuevo decimal(6,2)
as update empleados set sueldo=@sueldonuevo where sueldo=@sueldoanterior;
...
$200 si el sueldo es menor a $500 y $100, si el sueldo es mayor o igual a $500). Coloque como valor  por defecto para el parámetro el patrón "%".*/

create procedure pa_sueldototal @documento varchar(8) = '%' as select nombre,apellido,
sueldototal= 
case 
...
sección y que nos retorne el promedio de sueldos de todos los empleados de esa sección y el valor mayor de sueldo (de esa sección)*/
if object_id('pa_seccion') is not null
drop procedure pa_seccion;

create procedure pa_seccion @seccion varchar(20)='%', @promedio decimal(6,2) output, @mayor decimal(6,2) output
as  select @promedio=avg(sueldo) from empleados where seccion like @seccion
...
los valores devueltos por el procedimiento. Calcula sobre todos los registros porque toma el valor por defecto.*/
...
create procedure pa_sueldototal @documento varchar(8)='%',@sueldototal decimal(8,2) output
as select @sueldototal=
case 
	when sueldo<500 then sueldo+(cantidadhijos*200)
	when sueldo>=500 then sueldo+(cantidadhijos*100)
end
...
declare @st decimal(8,2 exec pa_sueldototal '22666666', @st outpu select @st;
...
como parámetro. Si no se ingresa un valor, o se ingresa "null", se muestra un mensaje y se sale del procedimiento.*/
create procedure pa_empleados_seccion @seccion varchar(20)=null
as if @seccion is null
	begin 
...
(si se ingresan valores para ambos parámetros) y "0", en caso que uno o ambos parámetros no se ingresen o 
sean nulos.*/
create procedure pa_actualizarhijos @documento char(8)=null, @hijos tinyint=null
as if (@documento is null) or (@hijos is null)
	return 0
else 
	begin
...
procedimiento enviando los dos parámetros y vea el contenido de la variable. El procedimiento retorna "1", con lo cual indica que fue actualizado.*/
declare @retorno int exec @retorno=pa_actualizarhijos '22222222',3
...
declare @retorno int exec @retorno=pa_actualizarhijos '22333333'
select 'Registro actualizado=1' = @retorno;
...
declare @retorno int exec @retorno=pa_actualizarhijos '22333333', 2
...
declare @retorno int exec @retorno=pa_actualizarhijos @hijos=4
...
código y nombre de la ciudad a la cual pertenece, de la ciudad de "Cordoba" empleando "with check option".*/
create view vista_clientes as
	select apellido, cl.nombre, documento, domicilio, cl.codigociudad,ci.nombre as ciudad
from clientes as cl join ciudades as ci on codigociudad=codigo where ci.nombre='Cordoba' with check option;
...
--9- Intente cambiar la ciudad de algún registro. Mensaje de error.
...
from visitas where numero=3 readtext visitas.comentarios @puntero 0 10;

--5- Intente leer el campo "comentarios" del registro número 1. Error, porque el puntero es inválido.
declare @puntero varbinary(16)
	select @puntero=textptr(comentarios) 
from visitas where numero=1 readtext visitas.comentarios @puntero 0 10;


--6- Recupere el campo "comentarios" de la visita número 1 (desde el comienzo al final), controlando que el puntero sea válido.
declare @puntero varbinary(16)
	select @puntero=textptr(comentarios) from visitas where numero=1
if (textvalid('visitas.comentarios', @puntero)=1) readtext visitas.comentarios @puntero 0 0
else 
	select 'puntero invalido';
...
sexo masculino, una corbata. La secretaria de la Gerencia necesita saber cuántos ramos de rosas y cuántas corbatas debe comprar para el mes de mayo.*/
...
recordatoria. La secretaria de Gerencia necesita saber la cantidad de años de servicio que cumplen los empleados que ingresaron en el mes de abril para encargar dichas placas.*/
...
from empleados where datepart(month,fechaingreso)=4;

/*6- La empresa paga un sueldo adicional por hijos a cargos. Para un sueldo menor o igual a $500 el  salario familiar por hijo es de $200, para un sueldo superior, el monto es de $100 por hijo. Muestre 
el nombre del empleado, el sueldo básico, la cantidad de hijos a cargo, el valor del salario por hijo, el valor total del salario familiar y el sueldo final con el salario familiar incluido de 
todos los empleados.*/
...
/*4- Es política de la empresa festejar cada fin de mes, los cumpleaños de todos los empleados que cumplen ese mes. Si los 
empleados son de sexo femenino, se les regala un ramo de rosas, si son de sexo masculino, una corbata. La secretaria de la Gerencia necesita saber
cuántos ramos de rosas y cuántas corbatas debe comprar para el mes de mayo.*/

if exists(select * from empleados
	where datepart(month,fechanacimiento)=5) (select sexo,count(*) as cantidad 
		from empleados where datepart(month,fechanacimiento)=5 group by sexo)
...
if exists (select * from entradas where capacidad>entradasvendidas)
	select sala,fechahora,capacidad-entradasvendidas as disponibles from entradas where capacidad>entradasvendidas
...
returns varchar(10) as begin declare @nombre varchar(10) set @nombre='Fecha inválida'  
...
returns varchar(5) as begin declare @nombre varchar(5) set @nombre='Fecha inválida'  
...
returns varchar(12) as begin declare @nombre varchar(12) set @nombre='Fecha inválida'   
...
incluyendo el día (emplee la función "f_nombreDia", la fecha (emplee la función "f_fecha"), el horario (emplee la función  "f_horario") y el nombre del paciente.*/
...
-- Cree la función "f_edad" que reciba 2 fechas (de tipo datetime) y nos retorne un valor positivo correspondiente a la diferencia entre ambas.
...
returns tinyint as begin declare @edad tinyint set @edad=null
...
select nombre, dbo.f_edad(fechaingreso,fechanacimiento) as 'edad al ingresar', dbo.f_edad(getdate(),fechaingreso) as 'años de servicio' from empleados;
...
--12- Muestre todos los empleados, empleando la función "f_valorNulo" enviándole como argumento los campos "mail" y "telefono".
select documento,nombre,fechanacimiento, dbo.f_valorNulo(telefono) as telefono, dbo.f_valorNulo(mail) as mail from empleados;

--74- Funciones de tabla de varias instrucciones ana empresa almacena los datos de sus empleados en una tabla denominada "empleados".
...
as begin
if @opcion not in ('total','parcial')  set @opcion='parcial'
if @opcion='total'
	insert @listado select documento, (apellido+' '+nombre), (domicilio+' ('+ciudad+')'), cast(fechanacimiento as varchar(12))from empleados
else
	insert @listado select documento,apellido,ciudad,cast(datepart(year,fechanacimiento) as char(4)) from empleados return
end;
